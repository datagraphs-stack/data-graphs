...
function renderVerify(evidence,revision){
  ...
  $('#verify').innerHTML=`<div class="verify"><div class="eyebrow">EVIDENCE · REVISION ${revision.number}</div><h2>Verify</h2><dl><dt>Question</dt><dd>${esc(revision.question)}</dd><dt>Interpretation</dt><dd>${revision.interpretation.status}${revision.interpretation.qualifications?.length?` · ${esc(revision.interpretation.qualifications.join(' '))}`:''}</dd><dt>Source</dt><dd>${esc(state.name)}</dd><dt>Rows</dt><dd>${evidence.sourceRows} source → ${evidence.filteredRows} after filters → ${evidence.resultRows} result</dd><dt>Metric semantic</dt><dd>${esc(metric.id)} v${metric.version} · ${esc(metric.label)}</dd><dt>Unit</dt><dd>${esc(metric.unit.kind)} · currency ${esc(metric.unit.currency)}</dd><dt>Status policy</dt><dd>Exclude ${esc(metric.statusPolicy.exclude.join(', '))}; include other statuses</dd><dt>Null policy</dt><dd>${esc(metric.nullPolicy)}</dd><dt>Columns used</dt><dd>${esc(evidence.columnsUsed.join(', '))}</dd><dt>Engine</dt><dd>${evidence.engineVersion}</dd><dt>Arithmetic</dt><dd>${esc(evidence.arithmetic)}</dd></dl><h3>Calculation</h3><ol>${steps}</ol>${evidence.warnings.length?`<div class="warning">${esc(evidence.warnings.join(' '))}</div>`:''}<details><summary>Validated structured plan</summary><pre>${esc(JSON.stringify(evidence.plan,null,2))}</pre></details></div>`;
}
