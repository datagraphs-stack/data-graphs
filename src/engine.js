...
export function validatePlan(plan,profile,semantics){
  ...
  for(const d of dimensions){if(!isRecord(d)||typeof d.field!=='string'){errors.push('Each dimension requires a field.');continue;}if(!(d.field in types))errors.push(`Unknown field: ${d.field}`);if(d.bucket!==undefined&&!['month','year'].includes(d.bucket))errors.push(`Unsupported date bucket: ${String(d.bucket)}`);if(d.bucket&&types[d.field]!=='date')errors.push(`Date bucketing requires a date field: ${d.field}`);}
...
export function executePlan(rows,plan,profile,semantics){const validation=validatePlan(plan,profile,semantics);if(!validation.valid)throw new Error(validation.errors.join(' ')); const filtered=rows.filter(r=>plan.filters.every(f=>f.operator==='eq'?r[f.field]===f.value:r[f.field]!==f.value)); const groups=new Map(); for(const row of filtered){const dims=plan.dimensions.map(d=>dimensionValue(row,d));const key=JSON.stringify(dims);if(!groups.has(key))groups.set(key,{dims,rows:[]});groups.get(key).rows.push(row);} let result=[...groups.values()].map(g=>{const out=Object.fromEntries(plan.dimensions.map((d,i)=>[d.bucket?`${d.field}_${d.bucket}`:d.field,g.dims[i]]));for(const m of plan.metrics){const vals=g.rows.map(r=>r[m.field]).filter(v=>strictNumber.test(v));const key=`${m.aggregation}_${m.field}`;out[key]=m.aggregation==='count'?g.rows.length:vals.length?aggregateDecimal(vals,m.aggregation):null;}return out;}); result=applyComparison(result,plan);for(const s of [...(plan.sort||[])].reverse())result.sort((a,b)=>(a[s.field]>b[s.field]?1:a[s.field]<b[s.field]?-1:0)*(s.direction==='asc'?1:-1));if(plan.limit)result=result.slice(0,plan.limit);const usedSemantics=plan.metrics.map(metric=>metricById(semantics,metric.semanticId));return {rows:result,evidence:{source:plan.source,sourceRows:rows.length,filteredRows:filtered.length,resultRows:result.length,columnsUsed:[...new Set([...plan.filters,...plan.dimensions,...plan.metrics].map(x=>x.field))],plan:structuredClone(plan),semantics:structuredClone(usedSemantics),semanticContextVersion:semantics.version,engineVersion:'0.4.0',arithmetic:'base-10 fixed-point aggregation; decimal scale derived per group',comparison:plan.comparison?{latestPeriod:result[0]?.[plan.comparison.periodDimension]??null,previousPeriod:result[0]?.previous_period??null,ranking:structuredClone(plan.ranking)}:null,warnings:[...profile.warnings,...usedSemantics.flatMap(metric=>metric.qualifications)]}};}
