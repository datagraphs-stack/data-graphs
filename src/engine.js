...
/** @typedef {{version:1,source:string,filters:Filter[],dimensions:{field:string,bucket?:'month'|'year'}[],metrics:{field:string,aggregation:'sum'|'count'|'average'|'min'|'max'}[],sort?:{field:string,direction:'asc'|'desc'}[],limit?:number,ranking?:{dimension:string,by:string,limit:number,period:'latest'},comparison?:{periodDimension:string,offset:'previous_year'},visualization:{type:'line'|'bar'|'table'}} AnalysisPlan */
