...
Phase A and meaningful parts of B–G exist: a dependency-free static web Studio, robust basic CSV parser, conservative profile, a versioned canonical metric semantic, versioned constrained AnalysisPlan, structural and semantic validation, fixed-point decimal aggregation, deterministic latest-period top-N and previous-year comparison, result chart/table, immutable in-session revisions, explicit clarification, and execution-derived Verify evidence. The engine emits an immutable versioned trace of source/semantic identities, ordered operations, meaningful stage counts, warnings, and runtime versions. Tests assert exact values and traces, semantic-policy enforcement, unknown currency, visible refund qualification, and that ambiguous intent cannot execute or produce a trace.
